...
public abstract class AptSnapshotFacetSupport extends FacetSupport implements AptSnapshotFacet { 
	...
	public void createSnapshot(String id, SnapshotComponentSelector selector) throws IOException {
		...
		for (SnapshotItem item : collectSnapshotItems(selector)) {
			...
			try (final TempStreamSupplier streamSupplier = new TempStreamSupplier(item.content.openInputStream())) {
				AssetBlob blob = tx.createBlob(
						item.specifier.path, 
						streamSupplier, 
						FacetHelper.hashAlgorithms, 
						null,
						FacetHelper.determineContentType(item), 
						true);
	...
	protected abstract List<SnapshotItem> fetchSnapshotItems(List<SnapshotItem.ContentSpecifier> specs) throws IOException;
}
