...
public class ControlFile {
	public static class Builder {
		...
		public Builder replaceParagraph(Predicate<Paragraph> predicate, Paragraph p) {
			paragraphs = Stream.concat(paragraphs.stream().filter(predicate), Stream.of(p))
	...
	public static class Paragraph {
		...
		public Paragraph withFields(List<ControlField> updateFields) {
			Map<String, ControlField> index = updateFields.stream()
					.collect(Collectors.toMap(f -> f.key, f -> f));
			return new Paragraph(
					Stream.concat(
							fields.stream().filter(f -> !index.containsKey(f.key)),
							updateFields.stream()
					).collect(Collectors.toList())
			);
		}
